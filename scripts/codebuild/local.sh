...
# to do the deploy. For local deploy, this assumes that the libcal-gateway-blueprints repo is a "sibling" to this
# project in your folder structure.
export BLUEPRINTS_DIR="../libcal-gateway-blueprints"
...
export STACK_NAME=${STACK_NAME:="libcal-gateway-${STAGE}"}
...
export GITHUB_REPO=$(git config --get remote.origin.url | sed 's/.*\/\([^ ]*\/[^.]*\).*/\1/') # ndlib/libcal-gateway
